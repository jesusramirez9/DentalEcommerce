        ...
        <div wire:model="size_id" class="divsize">
            @foreach ($sizes as $size)
                ...
                <div class="btn-group" role="group">
                    <input type="radio" class="btn-check" value="{{ $size->id }}" name="size"
                        id="{{ $size->name }}" autocomplete="off">
    ...
    <div class="text-xl text-gray-700 mt-4">
        ...
        <div wire:model="color_id" class="divsize">
            ...
            @forelse ($colors as $color)
                ...
                <div class="btn-group" role="group">
                    <input type="radio" class="btn-check" value="{{ $color->id }}" name="color"
                        id="{{ __($color->name) }}" autocomplete="off">
        ...
        <div class="flex-1">
            ...
            <div class="itemcolbtnweb">
                <x-button x-bind:disabled="!$wire.quantity" x-bind:disabled="$wire.qty > $wire.quantity"
                    ...
                    wire:target="addItem" color="orange" class="w-full agregar">
                    Agregar a la bolsa
                </x-button>
            </div>
            <div class="itemcolorbtn">
                <x-button x-bind:disabled="!$wire.quantity" x-bind:disabled="$wire.qty > $wire.quantity"
                     wire:click="addItem" wire:loading.attr="disabled"
                    wire:target="addItem" color="orange" class="w-full">
                    Agregar
                </x-button>
            </div>
